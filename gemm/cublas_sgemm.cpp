...
int main(int argc, char** argv)
{
  ...
  clock.tic();
  arma::fmat x(size, size, arma::fill::randu);
  arma::fmat y(size, size, arma::fill::randu);
  arma::fmat z(size, size); // Results.
  std::cout << "matrix initialization time: " << clock.toc() << "s\n";

  const size_t bufSize = size * size * sizeof(float);
  ...
  float alpha = 1.0;
  float beta = 0.0;
  stat = cublasSgemm_v2(handle, CUBLAS_OP_N, CUBLAS_OP_N, size, size, size, &alpha,
  ...
  stat = cublasGetMatrix(size, size, sizeof(float), devPtrC, size, z.memptr(),
      size);
  if (stat != CUBLAS_STATUS_SUCCESS)
  {
